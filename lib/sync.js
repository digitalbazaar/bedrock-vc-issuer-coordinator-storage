...
function _assertStatusUpdate(update) {
  try {
    ...
    assert.optionalString(
      update.status.indexAlloactor, 'update.status.indexAllocator');
    assert.object(
...
async function _updateStatus({update, signal} = {}) {
  ...
  try {
    ...
    await _updateRemoteStatus({
      credentialId, verifiableCredential, reference,
      updateStatusCapability, status
    });
...
async function _updateRemoteStatus({
  ...
  try {
    ...
    if(reference.indexAllocator !== undefined) {
      ...
      } else if(indexAllocator !== reference.indexAlloactor) {
